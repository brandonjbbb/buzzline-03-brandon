...
def process_message(message: str) -> None:
    ...
    try:
        ...
        # Extract your "text" field
        text = message_dict.get("text", "")
        logger.info(f"Message text: {text}")

        # Example real-time analytics: alert on keywords
        if "Hawaii" in text:
            logger.warning("ALERT: Someone mentioned Hawaii 🌴")

        if "dinner" in text:
            logger.info("Analytics: Counted a 'dinner' message")

        if "running" in text:
            logger.info("Analytics: Counted a 'running' message")
